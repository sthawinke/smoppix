...
nncrossFast = function (X, Y, dmax, iX = NULL, iY = NULL, what = c("dist", "which"),
          ...
          is.sorted.Y = FALSE, metric = NULL)
{
    stopifnot(is.ppp(Y) || is.psp(Y))
    if (!is.null(metric)) {
        ans <- invoke.metric(metric, "nncross.ppp", X = X, Y = Y,
                             iX = iX, iY = iY, what = what, ..., k = k, sortby = sortby,
                             is.sorted.X = is.sorted.X, is.sorted.Y = is.sorted.Y)
        return(ans)
    }
    ...
    if (!missing(k)) {
        if (length(k) == 0)
            ...
            if (any(k != round(k)) || any(k <= 0))
                stop(paste("some entries of the vector", sQuote("k"),
    ...
    if ((is.sorted.X || is.sorted.Y) && !(sortby %in% c("x",
                                                        "y")))
        stop(paste("If data are already sorted,", "the sorting coordinate must be specified explicitly",
                   "using sortby = \"x\" or \"y\""))
    switch(sortby, range = {
        WY <- as.owin(Y)
    ...
    if (kmaxcalc == 1) {
        ...
        if (!exclude)
            iX <- iY <- integer(1)
        z <- .C(spatstat.geom:::SG_knnXinterface, n1 = as.integer(nX), x1 = as.double(Xx),
                ...
                nnwhich = as.integer(nnwh), huge = as.double(huge),
        ...
        if (!exclude)
            iX <- iY <- integer(1)
        z <- .C(SG_knnXinterface, n1 = as.integer(nX), x1 = as.double(Xx),
                y1 = as.double(Xy), id1 = as.integer(iX), n2 = as.integer(nY),
                x2 = as.double(Yx), y2 = as.double(Yy), id2 = as.integer(iY),
                kmax = as.integer(kmaxcalc), exclude = as.integer(exclude),
                wantdist = as.integer(want.dist), wantwhich = as.integer(want.which),
                nnd = as.double(nndv), nnwhich = as.integer(nnwh),
                huge = as.double(huge), PACKAGE = "spatstat.geom")
        nnD <- z$nnd
        nnW <- z$nnwhich
        ...
        if (!is.sorted.Y)
            nnW <- oY[nnW]
        NND <- if (want.dist)
            matrix(nnD, nrow = nX, ncol = kmaxcalc, byrow = TRUE)
        else 0
        NNW <- if (want.which)
            matrix(nnW, nrow = nX, ncol = kmaxcalc, byrow = TRUE)
        else 0
        if (!is.sorted.X) {
            if (want.dist)
                NND[oX, ] <- NND
        ...
        if (kmax > kmaxcalc) {
            ...
            if (want.which)
                NNW <- cbind(NNW, matrix(NA_integer_, nrow = nX,
                                         ncol = kextra))
        }
        if (length(k) < kmax) {
            if (want.dist)
        ...
        colnames(result) <- c(if (want.dist) paste0("dist.",
                                                    k) else NULL, if (want.which) paste0("which.", k) else NULL)
        if (ncol(result) == 1)
            result <- result[, , drop = TRUE]
        return(result)
    }
