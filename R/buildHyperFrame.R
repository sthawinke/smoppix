...
setMethod("buildHyperFrame", "data.frame", function(x, coordVars, imageVars, coVars = setdiff(names(x),
...
setMethod("buildHyperFrame", "matrix", function(x, imageVars, covariates, ...) {
    ...
    desMat <- matrix(simplify2array(strsplit(names(ppps), "_")), nrow = nrow(hypFrame),
        byrow = TRUE)
...
setMethod("buildHyperFrame", "list", function(x, coordVars = c("x", "y"), covariatesDf = NULL,
    idVar = NULL, ...) {
    ...
    if (all(vapply(x, is.ppp, FUN.VALUE = TRUE))) {
        ...
        hypFrame <- spatstat.geom::hyperframe(ppp = lapply(x, function(z) {
            ...
            spatstat.geom::ppp(x = z[i, coordVars[1]], y = z[i, coordVars[2]], marks = z[i,
                PPcovariates, drop = FALSE], xrange = range(z[, coordVars[1]]), yrange = range(z[,
                coordVars[2]]), drop = FALSE)
        }), image = names(x))
    } else {
...
setMethod("buildHyperFrame", "SpatialExperiment", function(x, imageVars, coVars,
    ...
    buildHyperFrame(spatialCoords(x), image = as.data.frame(colData(x)[, imageVars,
        drop = FALSE]), covariates = cbind(gene = rownames(colData(x)), as.data.frame(colData(x))[,
        coVars, drop = FALSE]))
})
