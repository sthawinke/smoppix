...
addWeightFunction <- function(resList, pis = resList$pis, designVars, lowestLevelVar,
    ...
    Wfs <- lapply(pis, function(pi) {
        ...
        varEls <- lapply(features, function(gene) {
            ...
            if (cellId) {
                ...
                # Points with nn
                piList <- vapply(resList$hypFrame$pimRes[ordDesign], FUN.VALUE = double(1),
                  function(x) {
        ...
        varElMat <- matrix(unlist(varEls), ncol = if (pairId) {
            ...
            2
        }, byrow = TRUE, dimnames = list(NULL, c("quadDeps", if (pairId) c("minP",
            "maxP") else "NP")))
    ...
    names(Wfs) <- pis
    return(c(resList[setdiff(names(resList), c("Wfs", "designVars"))], list(Wfs = Wfs,
