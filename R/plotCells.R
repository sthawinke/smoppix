...
plotCells <- function(obj, features = getFeatures(obj)[seq_len(3)], nCells = 100, Cex = 1.5, borderColVar = NULL, borderCols = rev(palette()),
    warnPosition = TRUE, ...) {
    ...
    for (i in seq_along(tablesCell)) {
        ...
        for (j in seq_along(tablesCell[[nam]])) {
            ...
            shifted <- toUnitSquare(obj[[nam, "owins"]][[namIn]], ppp = subset.ppp(ppp, cell == namIn), Shift = Shift <- c(counter%%Ceil,
                counter%/%Ceil))
