...
hypFrame <- buildHyperFrame(df, coordVars = c("x", "y"), imageVars = c("condition",
    "fov"))
...
piEstsBG2 <- estPis(hypFrame2[, c("ppp", "image", "tabObs")], pis = "nn", null = "background",
    verbose = FALSE)
...
# Fit Yang models too
data(Yang)
hypYang <- buildHyperFrame(Yang, coordVars = c("x", "y"), imageVars = c("day", "root",
    "section"))
yangPims <- estPis(hypYang, features = getFeatures(hypYang)[seq_len(10)], pis = c("nn",
    "nnPair"), verbose = FALSE, nPointsAll = 10000)
