...
test_that("Reading in data proceeds without errors", {
    ...
    expect_message(hypFrame <- buildHyperFrame(df, coordVars = c("x", "y"), imageVars = c("fov",
        "condition")))
    expect_s3_class(hypFrame, c("hyperframe", "list"))
    expect_message(buildHyperFrame(as.matrix(df[, c("x", "y")]), image = df$fov,
        covariates = df[, c("gene", "condition"), drop = FALSE]))
    expect_message(hypFrame2 <- buildHyperFrame(as.matrix(df[, c("x", "y")]), image = df[,
        c("fov", "condition")], covariates = df[, "gene", drop = FALSE]))
    expect_identical(hypFrame, hypFrame2)
    expect_silent(hypFrame3 <- buildHyperFrame(lapply(listPPP, identity)))
    expect_message(hypFrame4 <- buildHyperFrame(split(df, f = apply(df[, c("fov",
        "condition")], 1, paste, collapse = "_")), covariatesDf = df[!duplicated(df[,
        c("fov", "condition")]), c("fov", "condition")]))
})
...
test_that("Adding regions of interest works", {
    ...
    expect_s3_class(hypFrame6 <- addCell(hypFrame, wList, cellTypes = cellTypesDf),
        "hyperframe")
    ...
    expect_error(hypFrame4 <- buildHyperFrame(spe, imageVars = c("sample_id", "in_tissue"),
        coVars = "in_tissue"))
})
