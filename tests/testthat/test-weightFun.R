...
test_that("Adding the weight function works", {
  ...
  expect_silent(objCSR2 <- addWeightFunction(piEstsCSR,
    pi = c("nnCell", "nnCellPair", "allDistCell", "allDistPairCell")
  ))
  ...
  expect_type(predNew <- evalWeightFunction(objBG$Wfs[["nn"]],
                            newdata = data.frame("NP" = 5)), "double")
  expect_true(predNew > evalWeightFunction(objBG$Wfs[["nn"]],
                                           newdata = data.frame("NP" = 4)))
  # With information sharing across features
  expect_s3_class(dfBG1 <- buildDfMM(objBG, gene = "gene1", pi = "nn"),
                  "data.frame")
  expect_s3_class(
...
test_that("Weight function application throws errors where appropriate", {
  ...
  expect_error(addWeightFunction(piEstsCSR, designVars = "condition",
                                 lowestLevelVar = "fov"))
})
