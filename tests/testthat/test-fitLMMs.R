...
test_that("Fitting linear mixed models proceeds without errors", {
    ...
    expect_message(fitLMMs(yangObj, pi = "nn",
    ...
    expect_message(linModsNNPair <- fitLMMs(yangObj, fixedVars = "day",
                                            pi = "nnPair"))
    expect_message(linMModsNN <- fitLMMs(yangObj, fixedVars = "day",
                                         randomVars = "root", pi = "nn"))
    expect_message(linMModsNNPair <- fitLMMs(yangObj, fixedVars = "day",
        randomVars = "root", pi = "nnPair"))
    # Returning the models
    expect_message(linModsNNfull <- fitLMMs(yangObj, fixedVars = "day",
                                            pi = "nn", returnModels = TRUE))
    expect_message(linMModsNNfull <- fitLMMs(yangObj, fixedVars = "day",
    ...
    expect_message(linModsMP <- fitLMMs(objBG, fixedVars = "condition",
                                        pi = "midpoint"))
    expect_message(linModsEdge <- fitLMMs(objBG, fixedVars = "condition",
                                          pi = "edge"))
...
test_that("Fitting linear mixed models throws errors where appropriate", {
    expect_error(fitLMMs(objBG, fixedVars = "condition", randomVars = "fov",
                         pi = "midpoint"))
})
