...
test_that("Fitting linear mixed models proceeds without errors", {
    expect_is(linModsNNint <- fitLMMs(yangPims, pi = "nn", features = getFeatures(yangPims)[1:5]),
        "list")
    expect_is(fitLMMs(yangPims, pi = "nn", verbose = FALSE, features = getFeatures(yangPims)[1:5]),
        "list")
    # Supply your own formula
    expect_is(fitLMMs(yangPims, features = getFeatures(yangPims)[1:5], pi = "nn",
        Formula = "pi - 0.5 ~ day +1|root"), "list")
    expect_is(linModsNNPairint <- fitLMMs(yangPims, features = getFeatures(yangPims)[1:5],
        pi = "nnPair"), "list")
    expect_is(linModsNN <- fitLMMs(yangPims, features = getFeatures(yangPims)[1:5],
        fixedVars = "day", pi = "nn"), "list")
    expect_is(linModsNNPair <- fitLMMs(yangPims, features = getFeatures(yangPims)[1:5],
        fixedVars = "day", pi = "nnPair"), "list")
    expect_is(linMModsNN <- fitLMMs(yangPims, features = getFeatures(yangPims)[1:5],
        fixedVars = "day", randomVars = "root", pi = "nn"), "list")
    expect_is(linMModsNNPair <- fitLMMs(yangPims, fixedVars = "day", features = getFeatures(yangPims)[1:5],
        randomVars = "root", pi = "nnPair"), "list")
    # Returning the models
    expect_is(linModsNNfull <- fitLMMs(yangPims, features = getFeatures(yangPims)[1:5],
        fixedVars = "day", pi = "nn", returnModels = TRUE), "list")
    ...
    expect_is(linModsEdge <- fitLMMs(objBG, features = getFeatures(objBG)[1:5], fixedVars = "condition",
        pi = "edge"), "list")
