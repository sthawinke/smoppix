...
test_that("Fitting linear mixed models proceeds without errors", {
    ...
    expect_is(
        linModsNNPair <- fitLMMs(yangPims, features = getFeatures(yangPims)[1:5], fixedVars = "day", pi = "nnPair"),
        "list"
    )
    expect_is(linMModsNN <- fitLMMs(yangPims,
        features = getFeatures(yangPims)[1:5], fixedVars = "day", randomVars = "root",
        pi = "nn"
    ), "list")
    expect_is(linMModsNNPair <- fitLMMs(yangPims,
        fixedVars = "day", features = getFeatures(yangPims)[1:5], randomVars = "root",
        pi = "nnPair"
    ), "list")
    # Returning the models
    expect_is(
        linModsNNfull <- fitLMMs(yangPims, features = getFeatures(yangPims)[1:5], fixedVars = "day", pi = "nn", returnModels = TRUE),
        "list"
    )
    ...
    expect_is(linModsMPcell <- fitLMMs(objBG,
        features = getFeatures(objBG)[1:5], fixedVars = c("condition", "cellType"),
        pi = "centroid"
    ), "list")
    # Account for cell as random effect
    expect_is(linModsEdgeCell <- fitLMMs(objBG,
        features = getFeatures(objBG)[1:5], fixedVars = "condition", randomVars = "image/cell",
        pi = "edge"
    ), "list")
    expect_is(linModsMidCellType <- fitLMMs(objBG, features = getFeatures(objBG)[1:5], addMoransI = TRUE, fixedVars = c(
        "condition", "cellType"), pi = "centroid", returnModels = TRUE, ), "list")
