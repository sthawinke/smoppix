...
test_that("Building data frames for mixed modelling proceeds without errors", {
    ...
    expect_s3_class(dfBG3 <- buildDataFrame(objBG, gene = "gene1", pi = "allDist"), "data.frame")
    expect_s3_class(dfBG4 <- buildDataFrame(objBG, gene = "gene1--gene2", pi = "allDistPair"), "data.frame")
    expect_silent(dfBG5 <- buildDataFrame(objBG, gene = "gene1", pi = "midpoint"))
    ...
    expect_s3_class(dfBG8 <- buildDataFrame(objBG, gene = "gene1", pi = "allDistCell"), "data.frame")
    expect_s3_class(dfBG9 <- buildDataFrame(objBG, gene = "gene1--gene2", pi = "nnPairCell"), "data.frame")
    expect_s3_class(dfBG10 <- buildDataFrame(objBG, gene = "gene1--gene2", pi = "allDistPairCell"), "data.frame")
    expect_silent(dfCSR1 <- buildDataFrame(objCSR, gene = "gene23", pi = "edge"))
    expect_silent(dfCSR3 <- buildDataFrame(objCSR, gene = "gene2", pi = "midpoint"))
    expect_s3_class(dfCSR4 <- buildDataFrame(objCSR, gene = "gene2", pi = "allDist"), "data.frame")
    expect_s3_class(dfCSR5 <- buildDataFrame(objCSR, gene = "gene20", pi = "nn"), "data.frame")
    expect_s3_class(dfCSR6 <- buildDataFrame(objCSR, gene = "gene2--gene20", pi = "nnPair"), "data.frame")
    expect_s3_class(dfCSR7 <- buildDataFrame(objCSR, gene = "gene2--gene23", pi = "allDistPair"), "data.frame")
    expect_s3_class(dfCSR8 <- buildDataFrame(objCSR, gene = "gene1", pi = "nnCell"), "data.frame")
    expect_s3_class(dfCSR9 <- buildDataFrame(objCSR, gene = "gene1", pi = "allDistCell"), "data.frame")
    expect_s3_class(dfCSR9 <- buildDataFrame(objCSR, gene = "gene1--gene2", pi = "nnPairCell"), "data.frame")
    expect_s3_class(dfCSR11 <- buildDataFrame(objCSR, gene = "gene1--gene2", pi = "allDistPairCell"), "data.frame")
...
test_that("Building data frames throws errors where appropriate", {
    ...
    expect_error(buildDataFrame(objCSR2, gene = "gene1", pi = "allDist"))
