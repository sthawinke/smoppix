...
nnObj <- estPis(hypYang,
    ...
    features = c("SmVND2", "SmPINR", getFeatures(hypYang)[seq_len(20)])
)
```
...
Probabilistic indices are features extracted from point patterns that can serve purposes other than hypothesis testing too. Here we demonstrate their use as interpretable predictors for predicting tumor type from cell type locations on a breast cancer dataset by \textcite{Keren2018}, previously analysed by \textcite{Vanderdoes2023}.
...
```{r plotTNBC, fig.cap = "Explorative plot of triple negative breast cancer dataset.\\parencite{Keren2018}"}
if(reqFunky){
...
if(reqFunky){
    ...
    if(require(glmnet)){
        ...
        lassoModel #In-sample misclassification error of 9,1%
    }
}
```
