...
This vignette demonstrates the use of the __smoppix__ package for statistical inference on single-molecule spatial transcriptomics datasets with replication.
...
```{r a, fig.height = 6.5}
plotTopResults(hypEng, results = allModsNNcell, pi = "nnPairCell", numFeats = 1, 
               what = "anti")
```
...
plotExplore(hypEng, piEsts = engPis, piColourCell = "edge", features = 
...
pValsGrads = getPvaluesGradient(engGrads)
```
