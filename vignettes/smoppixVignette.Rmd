...
nnObj <- estPis(hypYang,
    pis = c("nn", "nnPair"), null = "background", features = c("SmVND2", "SmPINR", getFeatures(hypYang)[seq_len(20)])
)
```
...
plotTopResults(hypEng, results = allModsNNcell, pi = "edge", what = "far", numFeats = 1)
```
...
```{r, fig.height = 6.5}
plotTopResults(hypEng, results = allModsNNcell, pi = "nnPairCell", numFeats = 1, what = "anti")
```
