...
Various errors may occur while fitting, e.g. insufficient non-missing observations. The function finishes silently but returns the error messages in the output. 
...
Again we fit and plot the weighting function, which is now a bivariate spline as a function of the minority and majority gene:
...
```{r}
dfBiNN = buildDfMM(nnPairObj, gene = "SmVND2--SmPINR", pi  = "nnPair", hypFrame = hypYang, 
                   weightFunction = wfNNpair, designVars = c("day", "root"))
```

Finally for the mixed model:

```{r}
lmeModNN = lmer(pi - 0.5 ~ day + (1|root), data = dfBiNN, na.action = na.omit, 
                                 weights = weight, contrasts = list("day" = "contr.sum"))
summary(lmeModNN)
```
