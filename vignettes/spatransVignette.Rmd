...
As example dataset we use the data used in a ["manuscript"]( 	
doi:10.1016/j.cub.2023.08.030) by Yang et al. 2023. on _Selaginella moellendorffii_. Only a subset of the data, consisting of sections 1-3 of roots 1-3 is included for computational reasons. The data is loaded as follows.
...
The first round of tests concerns the aggregation or regularity of individual transcripts. We choose to work with nearest neighbour distances here (pi = 'nn'). The background of all other transcripts. serves as null distribution, i.e. we look for transcripts aggregated _with respect to_ all other transcripts. As this is computationally most efficient, we also fit the PI for the bivariate analysis in this step ('nnPair', see below). We set the _nPointsAll_ parameter, which drives the resolution of the PI, a bit lower than default to limit compilation time of the vignette. 

```{r estpisyang, include = FALSE}
...
We find a significant positive intercept `r summary(lmeModNN)$coef["(Intercept)", "Estimate"]+0.5`, indicating strong antilocalization. And similarly, this process can be wrapped using _fitLMMs()_, as it has already been calculated:
...
A side question that may arise is whether the measures of intracellular aggregation have a global spatial patterning throughout the tissue. As a start, we can explore this graphically for the gene most remote from the cell edge:

```{r exploreSpatCell}
plotExplore(hypEng, piEsts = engPis, piColourCell = "edge", features = remEdge <- rownames(getResults(allModsNNcell$edge, "Intercept"))[1])
```

The molecules are shown in red, while the yellow-blue scale indicates the PI estikmte. It seems like in the central cells, the PI is smaller (molecules less remote from cell edge) than the outer cells. We test whether the Moran's I statistic, a measure for spatial aggregation of numeric variables, is significant over the different point patterns:
