...
```{r, include = FALSE}
allModsNN <- fitLMMs(nnObj, fixedVars = "day", randomVars = "root", pi = "nn")
```
...
We find a significant positive intercept `r summary(lmeModNN)$coef["(Intercept)", "Estimate"]+0.5`, indicating strong antilocalization. And similarly, this process can be wrapped using _fitLMMs()_:

```{r fitlmmPair}
allModsNNpair <- fitLMMs(nnObj, fixedVars = "day", randomVars = "root", pi = "nnPair")
head(getResults(allModsNNpair$nnPair, "Intercept"))
```
...
indicates insufficient memory. Try reducing the number of cores required with MultiCoreParam(),
