...
```{r}
library(lmerTest)
lmeMod = lmer(pi ~ day + (1|root), data = df, na.action = na.omit, offset = Offset, 
...
```{r}
allModsNN = fitLMMs(nnObj, pi  = "nn", hypFrame = hypYang, weightFunction = wfNN, designVars = c("day", "root"))
